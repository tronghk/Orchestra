						...
						// while($row=$playlist->fetch_assoc()):
							...
							if(count($list) > 0){
								foreach($list as $value){
									$idPl = $value['id'];
									$query = "select song.name from details join song on details.idSong = song.idSong where idPlaylist = '$idPl' limit 3";
