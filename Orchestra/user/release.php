...
?>
    ...
    <div class="player">
        ...
        <div class="player__content">
            ...
            <span class="player__track"><b class="player__title" id="player__title">Epic Cinematic</b> – <span
                    class="player__artist" id="player__artist">AudioPizza</span></span>  
            <audio src="" 
                id="audio" type='audio/mp3' controls></audio>
        </div>
    </div>
            ...
            <div class="row row--grid">
                ...
                <div class="col-12">
                    <ul class="breadcrumb bread">
                        ...
                        <li class="breadcrumb__item"><a href="#" id="loadrel">Releases</a></li>
                ...
                <div class="col-12">
                    <div class="release">
                        ...
                        <?php 
                            ...
                            // Check if the add_to_favorites button was clicked
                        if (isset($_GET['add_to_favorites'])) {
                            // Get the song ID from the POST data
                            $idSong = $_GET['idSong'];

                            // Add the song to the user's favorites list
                            $sql ="INSERT INTO `favorites`(`idSong`) VALUES ('$idSong')";  
                            $conn->query($sql); 
                        } 
                    ?>  
    ...
    $("#loadsong").click(function(e) {
        e.preventDefault();
        $("#page_content").load("song-details.php");
    });
  ...
  if(isset($_GET['createPlayList'])){
    $idNew = $_GET['createPlayList'];
    $idSong = $_GET['idSong'];
    echo'
    <form action="release.php" method="post" class="form-add align-content-center justify-content-center " style="display: flex;">
    <div class="form-group add-form" >
         <a href="release.php" >
            <i class="out bi bi-x-circle-fill"></i>
       </a>
        <h1 class="card-title text-center m-lg-3 mt-0" style="color: #000;" >
            Playlist mới
        </h1>
        <input type="hidden" name="idpl" value="'.$idNew.'">
        <input type="hidden" name="song" value="'.$idSong.'">
        <span >Tên playlist :</span> <input name="nameadd" required="required" value="" type="text">

        <div class="optionAdd">
            <div class="d-block text-center">
                Lựa chọn: <br>
                <button  type="submit"  class="load-form btn btn-primary btn-round ml-auto text-center " style="border-radius: 10px;
                background-color: blue;;width: 150px; height: 40px; margin-top: 15px;">
...
if(isset($_POST['nameadd'])){
    $idPl = $_POST['idpl'];
    $name = $_POST['nameadd'];
    $idCus = $_SESSION['email'];
    $song = $_POST['song'];
    $query = "insert into playlist(id,name,image,idCustomer) values($idPl,'$name','','$idCus')";
    $conn->query($query);
    $query = "insert into details(idPlaylist,idSong) values($idPl,'$song')";
    $conn->query($query);
    $_SESSION['messagePL'] = "Thêm playlist thành công";
}
?>
<?php
     if(isset($_SESSION['messagePL']) ){
        echo'
                        <div class="message-action" style="width:100%;display:flex;justify-content:center; height:200px ">
                        <div id="msgAlert" class="panel-body text-center" style="position: fixed;top:0;z-index: 10001; width:100%;background-color:white;text-align:center;">
                            '.$_SESSION['messagePL'].'
    ...
    if(isset($_GET['idPlaylist'])){
        ...
        $query  = "insert into details(idPlaylist,idSong) values('$idPlaylist','$idSong')";
