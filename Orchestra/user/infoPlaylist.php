...
?>
    ...
    <!-- main content -->
    <!-- footer -->
    <div class="perform_Song-List personal-listSong text-center" style="margin-left: 278px; background-color:#25a56a">
    <?php 
        if(isset($_GET['getplaylist'])){
            $name = $_GET['getplaylist'];
            $idP = $_GET['id'];
            $listP = array();
            $query = "select song.idSong as idSong, song.image as image , song.name as nameS, song.idArtist as idArtist, song.link as link from details join song on details.idSong = song.idSong where idPlaylist = $idP";
            ...
            if($result->num_rows > 0){
                while($row = $result->fetch_assoc()){
                    ...
                    $ob['image'] = $row['image'];
                    $idA = $row['idArtist'];
                    $query = "select * from artist where idArtist = $idA";
                    $data = $conn->query($query);
                    $r = $data->fetch_assoc();
                    $nameArtist = $r['name'];
                    $ob['nameAr'] = $nameArtist;
                    array_push($listP,$ob);
                }
            }
        }
    ?>
        <h1><?php echo $name ?></h1>
                                    ...
                                    <ul class="perform_Song-list_cart">
                                        <?php
                                            foreach($listP as $value){
                                                ...
                                                $lNameA = $value['nameAr'];
                                                $image = $value['image'];
                                                $linkM = "changeSong('/music/$l','$lName','$lNameA');";

                                                echo '
                                                
                                                <li onclick="'.$linkM.'" class="perform_Song-cart">
                                            ...
                                            <i id="" class="action_icon-play fa-solid fa-play "></i>
                                            </div>
                                            ...
                                            <a href="">'.$value['nameAr'].'</a>
                                            </div>
                                            <div class="perform_Song-cart-time">
                                            <i class="fa-solid fa-microphone"></i>
                                            <span>Time bài hát</span>
                                            </div>
                                            </li>
                                                ';
                                            }
